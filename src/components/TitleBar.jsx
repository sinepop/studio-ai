...
export default function TitleBar({ onOpenSettings, lang }) {
  ...
  const winBtn = (fn, icon, id, isClose) => {
    const hovered = hoveredBtn === id
    return (
      <button onClick={fn} style={{
        background: isClose && hovered ? '#E85454' : hovered ? 'rgba(255,255,255,0.08)' : 'transparent',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2, WebkitAppRegion: 'no-drag' }}>
        ...
        {winBtn(() => window.electronAPI?.minimize(), <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><line x1="2" y1="7" x2="12" y2="7"/></svg>, 'min')}
        {winBtn(() => window.electronAPI?.maximize(), isMax
          ...
          : <svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="10" height="10" rx="1.5"/></svg>, 'max')}
        {winBtn(() => window.electronAPI?.close(), <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><line x1="3" y1="3" x2="11" y2="11"/><line x1="11" y1="3" x2="3" y2="11"/></svg>, 'close', true)}
      </div>
    </div>
  )
}
